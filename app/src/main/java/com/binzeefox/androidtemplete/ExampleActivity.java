...
//    }
    ...
    public void startActivityRequest() {
        //直接请求所有内容，返回Observable
        RequestUtil.with(this).request(0x01).subscribe();
        //设置是否过滤不可开启文件以及设置文件类型
        RequestUtil.with(this).openable(true).type("image/*").request(0x02).subscribe();
        //跳转指定Intent并通过Rxjava返回结果
        Intent intent = new Intent(ExampleActivity.this, SecondActivity.class);
        RequestUtil.with(this).intentRequest(intent, 0x03).subscribe();
    }
